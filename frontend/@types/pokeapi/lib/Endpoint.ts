...
import type { ApiResourceList, Base } from "../Utility"
...
class Endpoint<T extends Base> {
  ...
  public async fetch(param: EndpointParam): Promise<T> {
    const data = await fetch(`${BASE_URI}/${this.resource}/${param}`).then(
      async (res) => res.json(),
    )

    if (!this._isT(data)) {
      throw new Error(
        `Invalid data received from ${BASE_URI}/${this.resource}/${param}`,
      )
    }
  ...
  public async list(
    ...
    const data = await fetch(`${BASE_URI}/${this.resource}?${params}`).then(
      async (res) => res.json(),
    )

    if (!this._isListT(data)) {
      throw new Error(
        `Invalid data received from ${BASE_URI}/${this.resource}?${params}`,
      )
    }
  ...
  public async listAll(cache: boolean = true): Promise<ApiResourceList<T>> {
    ...
    const first = await fetch(`${BASE_URI}/${this.resource}?limit=1`).then(
      async (res) => res.json(),
    )
    if (!this._isListT(first)) {
      throw new Error(
        `Invalid data received from ${BASE_URI}/${this.resource}?limit=1`,
      )
    }

    const data = await fetch(
      `${BASE_URI}/${this.resource}?limit=${first.count}`,
    ).then(async (res) => res.json())
    if (!this._isListT(data)) {
      throw new Error(
        `Invalid data received from ${BASE_URI}/${this.resource}?limit=${first.count}`,
      )
    }
